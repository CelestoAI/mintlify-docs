...
In this section, we will learn about how LLMs make use of tools and what are MCP Servers. You can skip to the next section to learn about [tool use with Agentor](./tool-use).
...
MCP (Model Context Protocol) is standard for communicating with LLMs that allows them to access external tools, APIs and other resources.
...
Let's understand the "why" behind MCP with a comparison between MCP and without MCP.

| **Without MCP** | **MCP** |
|-----------------|---------|
| Developers need to implement the tool calling logic in the application. | A prebuilt MCP Server can be just plugged in to the LLM to provide tools and APIs. |
