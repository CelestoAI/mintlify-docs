...
<Warning>
  0.1.0 rebuilt the agent engine and is a breaking change from the 0.0.x line — `DurableAgent` is gone and hosted tools were dropped. See the [0.1.0 release notes](https://github.com/CelestoAI/agentor/releases) for what changed and how to migrate.
</Warning>

To stay on the old 0.0.x line instead, pin it explicitly:

```bash
pip install "agentor<0.1"
```
...
| --- | --- | --- |
| `google` | `pip install "agentor[google]"` | [`GmailTool`](/agentor/api/tools/gmail), [`CalendarTool`](/agentor/api/tools/google-calendar), SuperAuth |
| `exa` | `pip install "agentor[exa]"` | `ExaSearchTool` |
| `git` | `pip install "agentor[git]"` | [`GitTool`](/agentor/api/tools/git) |
| `github` | `pip install "agentor[github]"` | [`GitHubTool`](/agentor/api/tools/github) |
| `postgres` | `pip install "agentor[postgres]"` | `PostgreSQLTool` |
| `slack` | `pip install "agentor[slack]"` | `SlackTool` |
| `scrapegraph` | `pip install "agentor[scrapegraph]"` | [`ScrapeGraphAI`](/agentor/api/tools/scrapegraphai) |
| `all` | `pip install "agentor[all]"` | Everything above |

<Info>
  **Google tools moved in 0.1.0.** The Google API client and SuperAuth are roughly 100 MB and used to be installed for everyone. They are now in the `google` extra. If `GmailTool` or `CalendarTool` raises an `ImportError`, run `pip install "agentor[google]"`.
</Info>
...
You should see `0.1.0` (or later), or a `0.0.x` version if you pinned the old line.
