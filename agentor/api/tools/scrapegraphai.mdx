...
<Info>
  The `scrapegraph` extra pulls in `scrapegraph-py>=2.1.0`, which requires **Python 3.12 or newer**. On Python 3.11, `pip install "agentor[scrapegraph]"` succeeds but the SDK is not installed, and instantiating `ScrapeGraphAI` raises `ImportError`.
</Info>
