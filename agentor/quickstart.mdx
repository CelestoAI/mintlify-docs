...
Fork any persisted run into a new, independent one — even a finished one, which `resume()` won't re-run. The original is never touched:

```python  theme={null}
variant = agent.fork(result.run_id, "How about Paris instead?")
print(variant.run_id)         # a new id — result.run_id is untouched
print(variant.final_output)
```

See [Durable runs](/agentor/durable-runs#fork-a-run) for what forking keeps and when to reach for it instead of `resume()`.
