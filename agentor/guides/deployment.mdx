...
description: "Deploy Agentor agents and MCP servers to the cloud using the Celesto CLI, with built-in observability, automatic scaling, and a single deploy command."
---
...
agent = Agentor(
    ...
    tools=[
        MCPServer(
            url="https://api.celesto.ai/deploy/apps/my-mcp-server/mcp",
            headers={"Authorization": f"Bearer {os.environ['CELESTO_API_KEY']}"},
            name="Deployed MCP",
        )
    ],
)
