...
<Note>
  vsock currently requires the QEMU backend on a Linux host with `vhost_vsock` loaded, and a sandbox image that ships the SmolVM guest agent. Both locally built images and the SmolVM-published images include it.
</Note>
...
<AccordionGroup>
  <Accordion title="comm_channel='vsock' raises on a Linux host">
    Make sure the `vhost_vsock` kernel module is loaded and the image you booted contains the SmolVM guest agent. Locally built images and current published images include it. If you're running a pinned older release of SmolVM, upgrade so it pulls images that bake the guest agent in.
