...
<Note>
  vsock currently requires the QEMU backend on a Linux host with `vhost_vsock` loaded, and a sandbox image that ships the SmolVM guest agent. Images built with `ImageBuilder` and SmolVM-published images include it. Images built with `DockerRootfsBuilder` need you to add the guest agent yourself.
</Note>
...
<AccordionGroup>
  <Accordion title="comm_channel='vsock' raises on a Linux host">
    Make sure the `vhost_vsock` kernel module is loaded and the image you booted contains the SmolVM guest agent. Images built with `ImageBuilder` and current published images include it. If you're booting a `DockerRootfsBuilder` image, add the guest agent yourself or use SSH instead. If you're running a pinned older release of SmolVM, upgrade so it pulls images that bake the guest agent in.
