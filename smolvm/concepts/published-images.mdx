...
The `ubuntu` row powers the Firecracker path for `smolvm create --os ubuntu`. It boots a clean Ubuntu rootfs in seconds when you want `apt` and a familiar Linux environment without any preset tooling. The QEMU path keeps using its qcow2 cloud image.
