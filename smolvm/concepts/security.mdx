...
The examples in this section assume `image` is a `BootImage` created from the [custom images guide](/smolvm/guides/custom-images).

### Isolated mode (default)

SmolVM defaults to `disk_mode="isolated"`, creating a per-VM disk from the base image:
...
- Creation overhead for copying or materializing the managed disk

You can resize an isolated custom-image disk without mutating the original image:
...
`grow_filesystem=True` currently supports raw ext4 images. For qcow2 images, SmolVM can resize the virtual disk, but the guest filesystem must be grown inside the guest.
...
<Warning>
  Shared mode means VMs boot directly from the same root filesystem image. Writes can mutate the base image, and concurrent writers can corrupt it. Use shared mode only for trusted workloads where you intentionally want that behavior.
</Warning>
...
Use `retain_disk_on_delete=True` on `VMConfig` when you need a lower-level retained-disk workflow. For most custom images, prefer isolated disks and snapshots for repeatable recovery.
