---
title: "smolvm env"
description: "Use smolvm env set, unset, and list commands to manage environment variables and secrets on a running sandbox without restarting the VM."
---
...
<Note>
  All `env` subcommands accept `--comm-channel {ssh,vsock}` to choose the host-to-guest control channel for that call. Leave it unset for auto-selection (vsock on Linux QEMU hosts with a recent image, otherwise SSH). See [control channel](/smolvm/concepts/control-channel).
</Note>
...
for vm_id in $(smolvm list --json | jq -r '.[].vm_id'); do
  echo "=== $vm_id ==="
