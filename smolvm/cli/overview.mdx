...
The `smolvm snapshot` command has subcommands for saving and restoring sandbox state:

| Command | Description |
| --- | --- |
| `smolvm snapshot create` | Create a snapshot from a running or paused VM |
...
The `smolvm browser` command has its own subcommands for managing disposable browser sandboxes:

| Command | Description |
| --- | --- |
| `smolvm browser start` | Create and start a browser sandbox |
| `smolvm browser list` | List browser sandboxes |
| `smolvm browser stop` | Stop and delete a browser sandbox |
| `smolvm browser open` | Open the viewer in your default browser |
| `smolvm browser logs` | Print browser sandbox logs |

Start a disposable browser sandbox with a viewer:

```bash
smolvm browser start --live --json
```

The JSON response includes the `session_id`, `viewer_url`, `display_url`, and browser automation URLs when available. Use the session ID with other browser commands:
