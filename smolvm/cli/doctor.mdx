---
title: "smolvm doctor"
description: "Run smolvm doctor to verify your host meets all SmolVM prerequisites — KVM access, required binaries, network tools, and the chosen Firecracker or QEMU backend."
---
...
* **Firecracker binary**: Located in the folder set by `SMOLVM_FIRECRACKER_DIR` if that variable is set, otherwise in `PATH` or `~/.smolvm/bin`
...
```
[FAIL] firecracker: Not found in PATH or ~/.smolvm/bin
```

**Solution**: Run the setup command to install Firecracker:

```bash  theme={null}
smolvm setup
```

If `SMOLVM_FIRECRACKER_DIR` is set, doctor only checks that folder and reports `Not found at <dir>/firecracker` instead. The suggested fix then includes the matching flag, for example `smolvm setup --firecracker-dir <dir>`.
