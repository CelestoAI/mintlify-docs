---
title: "smolvm snapshot"
description: "Create, restore, list, and delete SmolVM snapshots from the command line"
---

## Synopsis

```bash  theme={null}
smolvm snapshot {create,restore,delete,list} [OPTIONS]
```

## Description

The `snapshot` command saves and restores sandbox state. You can choose full, diff, or disk-only snapshot behavior depending on whether you need runtime state, smaller disk usage, or a cold-boot filesystem checkpoint.

## Subcommands

### create

Save a running or paused VM.

```bash  theme={null}
smolvm snapshot create <vm_id> [--snapshot-id <id>] [--snapshot-type <type>] [--resume-source] [--json]
```

<ParamField path="vm_id" type="string" required>
  The identifier of the VM to snapshot. The VM must be running or paused.
</ParamField>

<ParamField path="--snapshot-id" type="string">
  A custom identifier for the snapshot. If omitted, SmolVM generates one automatically (for example, `snap-my-vm-1717012345`). Must contain only lowercase letters, numbers, and hyphens.
</ParamField>

<ParamField path="--snapshot-type" type='"full" | "diff" | "disk"' default='"full"'>
  Snapshot type. Use `"full"` for a self-contained snapshot, `"diff"` to save changed disk data relative to the base image, or `"disk"` for a QEMU disk-only snapshot that cold-boots on restore.
</ParamField>

<ParamField path="--resume-source" type="flag">
  Resume the source VM after the snapshot is created. Without this flag, the VM stays paused.
</ParamField>

<ParamField path="--json" type="flag">
  Output machine-readable JSON instead of human-friendly text.
</ParamField>

**Example:**

```bash  theme={null}
smolvm snapshot create my-vm --snapshot-id before-deploy --snapshot-type disk --resume-source
```

**Example output:**

```
Created snapshot 'before-deploy' from VM 'my-vm'.
```

### restore

Restore a snapshot back into its original VM identity.

```bash  theme={null}
smolvm snapshot restore <snapshot_id> [--resume] [--force] [--json]
```

<ParamField path="snapshot_id" type="string" required>
  The identifier of the snapshot to restore.
</ParamField>

<ParamField path="--resume" type="flag">
  Resume the restored VM immediately. Without this flag, the VM is restored in a paused state.
</ParamField>

<ParamField path="--force" type="flag">
  Allow restoring a snapshot that was already restored before. By default, a snapshot can only be restored once.
</ParamField>

<ParamField path="--json" type="flag">
  Output machine-readable JSON instead of human-friendly text.
</ParamField>

**Example:**

```bash  theme={null}
smolvm snapshot restore before-deploy --resume
```

**Example output:**

```
Restored snapshot 'before-deploy' into VM 'my-vm'.
```

### delete

Delete a snapshot and its associated files.

```bash  theme={null}
smolvm snapshot delete <snapshot_id> [--json]
```

<ParamField path="snapshot_id" type="string" required>
  The identifier of the snapshot to delete.
</ParamField>

<ParamField path="--json" type="flag">
  Output machine-readable JSON instead of human-friendly text.
</ParamField>

<Warning>
  Deletion is permanent. The snapshot's saved artifacts cannot be recovered.
</Warning>

**Example:**

```bash  theme={null}
smolvm snapshot delete before-deploy
```

### list

List all snapshots, optionally filtered by source VM.

```bash  theme={null}
smolvm snapshot list [--vm-id <vm_id>] [--json]
```

<ParamField path="--vm-id" type="string">
  Filter the list to show only snapshots created from this VM.
</ParamField>

<ParamField path="--json" type="flag">
  Output machine-readable JSON instead of human-friendly text.
</ParamField>

**Example:**

```bash  theme={null}
smolvm snapshot list
```

**Example output:**

```
  Snapshot ID        VM ID     Restored   Created At
  before-deploy      my-vm     No         2026-04-03T12:30:00
  pre-experiment     agent-1   Yes        2026-04-03T11:15:00

Total: 2 snapshot(s).
```

Filter by VM:

```bash  theme={null}
smolvm snapshot list --vm-id my-vm
```

## JSON output

All subcommands support `--json` for scripting and automation. The JSON response includes snapshot metadata such as `snapshot_id`, `vm_id`, `snapshot_type`, `restored`, `created_at`, and artifact paths.

**Example:**

```bash  theme={null}
smolvm snapshot create my-vm --snapshot-id test-snap --json
```

```json  theme={null}
{
  "command": "snapshot.create",
  "exit_code": 0,
  "data": {
    "snapshot": {
      "snapshot_id": "test-snap",
      "vm_id": "my-vm",
      "snapshot_type": "full",
      "restored": false,
      "restored_vm_id": null,
      "created_at": "2026-04-03T12:30:00+00:00",
      "disk_path": "/home/user/.local/state/smolvm/snapshots/test-snap/disk.ext4"
    }
  }
}
```

## Exit codes

| Code | Description |
| ---- | --- |
| `0` | Success |
| `1` | Error (snapshot not found, VM not running, etc.) |
| `2` | Usage error (missing subcommand or arguments) |

## Related commands

* [`smolvm list`](/smolvm/cli/list) — See all running VMs
* [`smolvm create`](/smolvm/cli/create) — Create a new VM to snapshot
* [`smolvm ssh`](/smolvm/cli/ssh) — Connect to a restored VM
