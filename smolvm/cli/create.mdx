...
<ParamField path="--network" type="string" default="nat">
  Network mode for the sandbox. Choices are `nat` (default, private SmolVM network with NAT, port forwarding, and outbound network controls) and `bridge` (attach directly to an existing host bridge on Linux). See [Bridged networking](/smolvm/features/bridged-networking).
</ParamField>
...
In bridge mode, SSH from the host, host port forwards, workspace mounts, and SmolVM-managed outbound controls are unavailable — use `smolvm sandbox shell` and connect to guest services over the bridged network instead. See [Bridged networking](/smolvm/features/bridged-networking) for the full setup, tradeoffs, and SDK usage.
