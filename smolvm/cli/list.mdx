...
A preset is a ready-made sandbox setup for an agent. Use `--preset` to show only sandboxes created by one preset:
...
<Note>
  Preset filtering uses information saved when SmolVM creates a sandbox. Sandboxes created by older SmolVM releases and manually prepared sandboxes remain available through `smolvm sandbox list --all`, but do not appear in preset-filtered results.
</Note>
