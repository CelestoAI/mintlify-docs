...
Before rendering the table, `list` does a quick liveness check on every VM marked `running` or `paused`. If the underlying process is gone — for example, because the host was rebooted or the process was killed — the VM is shown as `error` instead of falsely appearing healthy. Run [`smolvm delete <vm_id>`](/smolvm/cli/cleanup) (or `smolvm cleanup`) to clear those rows.
