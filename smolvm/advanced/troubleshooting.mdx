...
<AccordionGroup>
  ...
  <Accordion title="No IP addresses available in pool">
    ...
    **Solution**:
    ...
    3. Or use the CLI cleanup command directly:
  ...
  <Accordion title="QEMU exited early while booting (macOS)">
    ...
    **Solution**:
    ...
    5. Rebuild the image to ensure kernel and rootfs are compatible:

    ```python  theme={null}
    from smolvm import SmolVM

    # Auto-config mode builds a matching kernel and rootfs
    with SmolVM() as vm:
  ...
  <Accordion title="VM stuck in ERROR state">
    ...
    **Solution**:
    ...
    with SmolVM() as new_vm:
        result = new_vm.run("echo 'recovered'")
        print(result.stdout.strip())
    ```

    4. Or clean up all stale VMs via CLI:
  ...
  <Accordion title="RuntimeError: Unable to find writable data directory">
    ...
    **Solution**:
    ...
    3. Set an explicit data directory:
