...
<AccordionGroup>
  ...
  <Accordion title="Firecracker binary not found">
    **Problem**: SmolVM cannot find the `firecracker` executable. SmolVM looks in the folder set by `SMOLVM_FIRECRACKER_DIR` (or the `--firecracker-dir` setup flag) if one is configured. Without a configured folder, it checks `PATH` first, then `~/.smolvm/bin/firecracker`.

    **Solution**:

    1. Run the setup command, which installs Firecracker to `~/.smolvm/bin` by default:
    ...
    ```bash  theme={null}
    smolvm doctor
    # The firecracker check should show the resolved path,
    # for example ~/.smolvm/bin/firecracker
    ```

    If you installed Firecracker to a custom folder that is not on `PATH`, export `SMOLVM_FIRECRACKER_DIR` so SmolVM can find it:
