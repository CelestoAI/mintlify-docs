...
vm = SmolVM(memory=512, disk_size=2048)

# Heavy workload
vm = SmolVM(memory=2048, disk_size=8192)
```
...
For workloads that need specific dependencies, build them into a custom image and launch that image with `SmolVM.from_image()`. This avoids reinstalling packages every time the sandbox starts.
...
image = DockerRootfsBuilder(
    ...
    dockerfile="""
FROM alpine:3.20
RUN apk add --no-cache python3 py3-requests py3-numpy
COPY init /init
RUN chmod +x /init
""",
    context={"init": "#!/bin/sh\nexec /bin/sh\n"},
...
The first build runs Docker. Later launches reuse the cached root filesystem unless the Dockerfile, build context, build args, platform, rootfs size, or fingerprint inputs change.
...
* Higher disk usage
* Copy or materialization overhead on create
...
```python  theme={null}
vm = SmolVM.from_image(image, disk_mode="shared")
```

Use shared mode only when the image is trusted and protected from concurrent writes.
