...
SmolVM separates the base image from the disk that a running VM writes to. In the default mode, the base image stays unchanged and each VM gets its own managed disk.

### Isolated Mode (Default)

Each VM gets a private disk materialized from the configured root filesystem:
...
1. SmolVM resolves the base image from `config.rootfs_path`.
2. It creates a per-VM disk under the state directory.
3. It applies `disk_size_mib` when a larger managed disk is requested.
4. It updates the effective VM config so the backend boots the managed disk.
5. It removes the managed disk on delete unless `retain_disk_on_delete=True`.

**Benefits**:

- Complete filesystem isolation between VMs
- No writes to the original image artifact
- Safer concurrent VM creation
- Transactional cleanup when create, resize, or restore fails

**Tradeoffs**:

- More disk usage than shared mode
- Extra work during create, especially for larger images

### Disk Resize And Filesystem Growth

SmolVM can resize managed disks during create when `disk_size_mib` is set.
...
Resize behavior depends on `rootfs_format`:

| Format | Managed disk behavior |
| --- | --- |
| `raw-ext4` | SmolVM can grow the disk file and run `e2fsck` plus `resize2fs` when `grow_filesystem=True`. |
| `qcow2` | SmolVM can resize the virtual disk with `qemu-img`, but it does not grow the guest filesystem. |

For QEMU raw ext4 images with `grow_filesystem=True`, SmolVM materializes a raw per-VM disk instead of a qcow2 overlay so host-side ext4 growth can work.

### Root Filesystem Format

`rootfs_format` tells SmolVM how to treat the base image.
...
QEMU uses this declared format for drive arguments, `qemu-img` backing format, resizing, and snapshot decisions. New configs should set `rootfs_format` explicitly instead of relying on filename suffixes.
...
config = VMConfig(
    kernel_path=ensure_base_kernel_for_backend("qemu"),
    rootfs_path=Path("/path/to/base.ext4"),
    rootfs_format="raw-ext4",
    boot_args="console=ttyS0 reboot=k panic=1 root=/dev/vda rw init=/init",
    backend="qemu",
    disk_mode="shared"
...
1. The backend mounts `config.rootfs_path` directly.
2. Writes land in the base image.
3. Multiple VMs can share the same filesystem state.

**Use cases**:

- Read-only workloads where the image is protected externally
- Trusted persistent development environments
- Lower disk usage

**Risks**:

- Cross-VM contamination
- Concurrent writes can corrupt the filesystem
- SmolVM refuses resize requests for shared base images
