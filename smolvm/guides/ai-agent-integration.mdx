...
SmolVM provides built-in browser sessions that run an isolated Chromium instance inside a microVM. You don't need to install a browser manually — just create a `BrowserSession` and connect over CDP:
...
# Usage
title = browse_url("https://example.com")
print(title)
```

For sessions that need visual inspection, use `mode="live"` to get a noVNC live-view URL alongside the CDP endpoint:
...
with BrowserSession(
    ...
    session.screenshot("artifacts/screenshot.png")
```

<Tip>
  See the [browser sessions guide](/smolvm/guides/browser-sessions) for persistent profiles,
  artifact collection, file transfer, and CLI usage.
</Tip>
