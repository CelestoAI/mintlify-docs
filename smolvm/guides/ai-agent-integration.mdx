...
3. The agent uses `agent-browser --cdp <cdp_port>` commands to take snapshots, click elements, and navigate pages.
...
agent = Agent(
    ...
    instructions=(
        ...
        "2. Read the `cdp_port` from the `parsed_browser_session` in the output.\n"
        "3. Use `agent-browser --cdp <cdp_port>` for all browser commands.\n"
