...
[Agentor](/agentor/home) is Celesto's own agent framework. It has built-in SmolVM support through `SmolVMRuntime`, so your agent's shell commands run inside a sandbox automatically — no glue code needed.
...
<Tip>
  See the full working example in [`main.py`](https://github.com/celestoai/smolvm/blob/main/main.py). Install with `pip install agentor smolvm`.
</Tip>
...
<Tip>
  See the full working example in [`examples/agent_tools/langchain_tool.py`](https://github.com/celestoai/smolvm/blob/main/examples/agent_tools/langchain_tool.py). Install with `pip install smolvm langchain-openai langgraph`.
</Tip>
