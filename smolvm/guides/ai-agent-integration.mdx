...
SmolVM has built-in support for disposable Chromium browser sessions. Instead of manually installing a browser inside a VM, use the `BrowserSession` class to get a ready-to-use browser with CDP and optional live-view:
...
with BrowserSession(
    ...
    # Connect Playwright for automation
...
Browser sessions support persistent profiles for workflows that need to maintain cookies and local storage across sessions:
...
<Tip>
  See the [browser sessions guide](/smolvm/guides/browser-sessions) for full configuration options and the [agent framework examples](/smolvm/guides/agent-framework-examples) for integration with LangChain, OpenAI Agents, and PydanticAI.
</Tip>
...
<CardGroup cols={2}>
  <Card title="Browser sessions" icon="globe" href="/smolvm/guides/browser-sessions">
    Run isolated Chromium sessions for web automation and computer use
  </Card>

  <Card title="Agent framework examples" icon="puzzle-piece" href="/smolvm/guides/agent-framework-examples">
    LangChain, OpenAI Agents SDK, and PydanticAI integrations
  </Card>
