...
[Agentor](/agentor/introduction) is Celesto's own agent framework. It has built-in SmolVM support through `SmolVMRuntime`, so your agent's shell commands run inside a sandbox automatically — no glue code needed.
