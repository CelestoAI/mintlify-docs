...
<Steps>
  ...
  <Step title="Create the sandbox">
    Create a sandbox named `test-mac`:
  ...
  <Step title="Open the desktop">
    After creation finishes, open the sandbox in Screen Sharing:
...
```bash
smolvm sandbox desktop test-mac --start
```
...
<AccordionGroup>
  <Accordion title="The confirmation prompt says invalid input">
    ...
    ```bash
    smolvm sandbox create --os macos --name test-mac --yes
    ```
  </Accordion>
  ...
  <Accordion title="Screen Sharing did not open">
    ...
    ```bash
    smolvm sandbox desktop test-mac --start --boot-timeout 120
    ```
  </Accordion>
