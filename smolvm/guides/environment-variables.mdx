...
```python  theme={null}
from smolvm import ImageBuilder, SmolVM, SSH_BOOT_ARGS, VMConfig
...
with SmolVM(config, ssh_password="smolvm") as vm:
...
<Note>
  Environment variable injection requires an SSH-capable image. Use auto-config mode, `ImageBuilder`, or a `BootImage` with `ssh_capable=True`.
</Note>
