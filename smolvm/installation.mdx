...
<Note>
  Linux may prompt for `sudo` during `smolvm setup` to install host packages (`nftables`, `iproute2`) and configure runtime permissions. Firecracker itself installs into your home directory at `~/.smolvm/bin`, so it needs no system folders. On macOS, setup installs QEMU via Homebrew.
</Note>
...
On Linux, `smolvm setup` installs Firecracker into `~/.smolvm/bin` for the user who runs the command. The install is atomic: SmolVM stages the binary in a temporary file and moves it into place, so a partially downloaded binary never replaces a working one. If a `firecracker` binary is already on your `PATH`, SmolVM keeps using it and does not install a duplicate.
...
If that folder is not on your `PATH`, set `SMOLVM_FIRECRACKER_DIR` so later SmolVM commands can find the binary:
...
SmolVM looks for Firecracker in this order:

1. The `--firecracker-dir` flag, if passed.
2. The `SMOLVM_FIRECRACKER_DIR` environment variable. When either of these is set, SmolVM only looks in that folder.
...
Silverblue, Bluefin, and other Fedora Atomic systems work with the normal `smolvm setup` command when the required host tools are already installed. SmolVM never modifies the rpm-ostree deployment itself. If a required tool is missing, setup prints the exact `sudo rpm-ostree install` command to run, and asks you to reboot before retrying.
...
<Tabs>
  <Tab title="Linux">
    - Ubuntu, Debian, or Fedora. `smolvm setup` installs missing host packages with `apt` or `dnf`. On rpm-ostree systems such as Fedora Atomic, it prints the `rpm-ostree` command to run instead. Other distributions work but you may need to install host dependencies yourself
