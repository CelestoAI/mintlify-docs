...
<Steps>
  ...
  <Step title="Run one-time setup">
    Set up the runtime dependencies for your machine:

    ```bash
    smolvm setup
    ```

    On Linux this installs Firecracker, configures KVM permissions, and sets up networking tools. Linux may prompt for `sudo` during setup.

    On macOS this installs QEMU via Homebrew. The QEMU backend is automatically detected.
  </Step>
