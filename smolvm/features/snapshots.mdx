...
When you create a snapshot, you can pick how much of the disk to store:

- **`full`** (default) — A complete, self-contained copy of the disk. Restores on its own even if the original base image is gone. Use this for everyday work and anytime you want a portable, no-surprises snapshot.
- **`diff`** — Stores only the data that changed since the shared base image, so it takes far less space. Use this in production systems that take many snapshots and keep their base images in place. A diff snapshot needs its base image to still be present to restore.
