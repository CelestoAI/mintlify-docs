...
<Steps>
  <Step title="Create an OpenClaw sandbox">
    ...
    SmolVM installs OpenClaw 2026.9.1 after the sandbox starts, so creating each new sandbox can take several minutes. SmolVM shows each installation step while you wait.
...
To print the one-time dashboard link instead of opening a browser:

```bash
smolvm openclaw open-ui openclaw-work --no-browser
```

Treat the link as a credential. Close the local connection with the exact `smolvm sandbox port close` command printed by `open-ui`.

Updating SmolVM leaves OpenClaw inside existing sandboxes unchanged. Before replacing an older sandbox, save a snapshot with [sandbox snapshots](/smolvm/cli/snapshot).
