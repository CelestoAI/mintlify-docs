...
Every hook receives one argument, which you'll see called `ctx` in the examples below. This is a **run context** — a single object that describes the command in flight: what was run, on which sandbox, and (afterwards) its result. You read from it to decide what to do. The [run context](#the-run-context) section lists every field; for now, just know that `ctx.command` is the command string and `ctx.vm_id` is the sandbox ID.
