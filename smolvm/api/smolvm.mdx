...
description: "SmolVM class reference: the Python API for creating microVM sandboxes, running commands over the selected control channel, and managing the full sandbox lifecycle."
---
...
SmolVM(
    ...
    *,
    ...
    qemu_machine: Literal["auto", "microvm", "q35"] = "auto",
...
<ParamField path="qemu_machine" type='Literal["auto", "microvm", "q35"]' default="auto">
  QEMU machine model. Keep `auto` for normal use so supported Linux guests use the faster `microvm` path.
</ParamField>
...
<ParamField path="internet_settings" type="InternetSettings | dict | None" default="None">
  Outbound access policy. See [Network controls](/smolvm/features/network-controls) for supported QEMU and Firecracker combinations.
</ParamField>

<ParamField path="mounts" type="list[str] | None" default="None">
  Host folders to share with the sandbox. Each entry uses `HOST_PATH[:GUEST_PATH]`.
</ParamField>

<ParamField path="writable_mounts" type="bool" default="False">
  Allow the sandbox to write through configured shared folders.
</ParamField>
...
<ParamField path="state_manager" type="StateManagerProtocol | None" default="None">
  Inventory used to store and find sandbox state. Pass the same instance to create, reconnect, and restore operations that need to share an inventory.
</ParamField>
...
Reconnect to an existing VM by ID through the same inventory that created it. Pass `ssh_password=` for Windows guests that use password-based SSH. Pass `comm_channel=` to override the host-to-guest control channel; see [control channel](/smolvm/concepts/control-channel).

<ParamField path="vm_id" type="str" required>
  VM identifier to reconnect to.
</ParamField>

<ParamField path="state_manager" type="StateManagerProtocol | None" required>
  Inventory that contains the sandbox. Reuse the exact state manager supplied when the sandbox was created.
</ParamField>
...
* `ValueError`: If `state_manager` is omitted.
...
<ParamField path="state_manager" type="StateManagerProtocol | None" default="None">
  Inventory used for the new sandbox. Pass the same instance to a later reconnect or snapshot restore.
</ParamField>
...
<ParamField path="qemu_machine" type='Literal["auto", "microvm", "q35"]' default="auto">
  QEMU machine model. Keep `auto` for the fastest supported default.
</ParamField>
...
<ParamField path="internet_settings" type="InternetSettings | dict | None" default="None">
  Outbound access policy. QEMU `off` works with the default network on macOS and Linux; `restricted` requires explicit TAP networking on Linux.
</ParamField>
...
<ParamField path="comm_channel" type='Literal["ssh", "vsock"] | None' default="None">
  Host-to-guest command channel for the restored sandbox.
</ParamField>

<ParamField path="state_manager" type="StateManagerProtocol | None" default="None">
  Inventory containing the snapshot. Reuse the state manager supplied to the workflow that created the snapshot.
</ParamField>
...
<ParamField path="guest_loopback" type="bool" default="False">
  Route to the guest's `127.0.0.1` through an SSH tunnel. Use this only when the application listens on guest loopback instead of its network address.
</ParamField>
...
inventory = MemoryStateManager(Path(".smolvm-state"))
original = SmolVM(state_manager=inventory)
original.start()
vm_id = original.vm_id
original.close()

# Reuse the same inventory when creating a new handle.
vm = SmolVM.from_id(vm_id, state_manager=inventory)
