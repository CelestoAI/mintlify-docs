---
title: "ImageBuilder"
description: "ImageBuilder reference: build minimal Alpine or Debian-based VM rootfs images with SSH pre-configured using Docker, complete with networking and init scripts."
---
...
<Note>
  Since v0.0.14, the auto-config path (`smolvm create --os ...`) supports `alpine` and `ubuntu` only — Debian was removed from the `GuestOS` enum. The `build_debian_ssh_key` method below remains callable when you instantiate `ImageBuilder` directly, but it is no longer wired into the CLI's `--os` flag.
</Note>
