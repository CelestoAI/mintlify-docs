...
<ParamField path="vm_id" type="str" default="auto-generated">
  Unique identifier for the VM. Must be lowercase alphanumeric with hyphens or underscores, matching the pattern `^[a-z0-9][a-z0-9_-]{0,62}[a-z0-9]$` or `^[a-z0-9]$`.
...
<ParamField path="extra_drives" type="list[Path]" default="[]">
  Additional block-device image paths to attach at boot. Each path must exist and point to a valid file. Use this when your VM needs extra storage volumes beyond the root filesystem.
</ParamField>
...
<ParamField path="network_rate_limit_mbps" type="int | None" default="None">
  Optional network bandwidth rate limit in megabits per second. When set, guest network traffic is throttled to the specified rate. Must be at least 1 Mbps. Leave as `None` for unrestricted bandwidth.
</ParamField>
...
| `network_rate_limit_mbps`  | `int \| None`    | `None`                                     | >= 1 (or `None` for unlimited)        |
