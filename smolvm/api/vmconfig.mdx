...
<ParamField path="extra_drives" type="list[Path]" default="[]">
  Additional block device images to attach to the VM. Each path must exist and point to a valid file.
</ParamField>
...
<ParamField path="network_rate_limit_mbps" type="int | None" default="None">
  Network bandwidth rate limit in megabits per second. When set, both inbound and outbound traffic are throttled using a token-bucket rate limiter. Must be at least 1 Mbps if specified.

  Only applies to the Firecracker backend.
</ParamField>
...
| `network_rate_limit_mbps`  | `int \| None`    | `None`                                     | >= 1 if set (Firecracker only)        |
