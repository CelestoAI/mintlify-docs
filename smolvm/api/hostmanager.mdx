---
title: "HostManager"
description: "HostManager reference: validate KVM access and host networking tools, install and pin the Firecracker binary, and prepare the SmolVM runtime environment."
---
...
<ParamField path="firecracker_dir" type="str | Path | None" default="None">
  Folder containing SmolVM's Firecracker binary. An explicit value overrides the `SMOLVM_FIRECRACKER_DIR` environment variable. When neither is set, the default is `~/.smolvm/bin`.
</ParamField>
...
# Use a custom Firecracker folder
host_mgr = HostManager(firecracker_dir="~/.local/bin")
```
...
Find the Firecracker binary. If a folder is configured through the `firecracker_dir` constructor argument or the `SMOLVM_FIRECRACKER_DIR` environment variable, only that folder is checked. Otherwise the search order is:
...
Downloads the official tarball, extracts the firecracker binary, and installs it to the configured folder (`~/.smolvm/bin/` by default).
