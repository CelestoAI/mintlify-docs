...
<Steps>
  ...
  <Step title="Start Pi from your project">
    ...
    The extension creates a Celesto computer with an empty `$HOME/workspace` and routes Pi's `read`, `write`, `edit`, and `bash` tools there. Computers created by the extension use a [persistent home](/cloud/features/persistence#keep-the-home-directory-across-restore), so `$HOME` survives stop and restore. Nothing is copied from your machine yet.
...
A caller-selected computer is never deleted automatically, and the extension does not change its persistence settings. It keeps whatever home persistence it was created with. Any files already in `$HOME/workspace` remain untouched until you explicitly run `/celesto push` or `/celesto sync`. A non-empty legacy `/workspace` is moved to `$HOME/workspace` automatically when the home workspace is empty.
