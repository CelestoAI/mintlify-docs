...
<ParamField body="persistent_home" type="boolean" default="false">
...
Computers are ephemeral by default. Pass `persistent_home=True` to keep `/home/ohm` across stop and restore:

```python create_persistent_computer.py
from celesto import Computer
...
See [Keep work between sessions](/cloud/features/persistence) for how persistence works.
...
<ParamField body="persistentHome" type="boolean" default="false">
  Keep `/home/ohm` across stop and restore. Set this at create time when you plan to reuse the computer across sessions. This setting cannot be changed after the computer is created.
</ParamField>
...
```ts create-persistent-computer.ts
import { Computer } from "@celestoai/sdk";
...
<ResponseField name="persistentHome" type="boolean">
  Whether `/home/ohm` is kept across stop and restore. Available in TypeScript as `persistentHome`. Set at create time and cannot be changed later.
</ResponseField>
