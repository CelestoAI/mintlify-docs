...
Create a computer, run one command, and delete it when you're done. By default, Celesto gives you a `scratch` computer — a minimal Ubuntu sandbox with no extra tools preinstalled.
...
If your agent needs common coding tools already installed, see [Use a template](#use-a-template).
...
Call `create()` with no arguments to get a fresh `scratch` computer with the default size. Use this when you just need a clean Ubuntu sandbox to run commands in.
...
print(computer["template_id"])  # "scratch"
```
...
console.log(computer.templateId); // "scratch"
```

</CodeGroup>

Override the default size by passing any of `cpus`, `memory`, or disk size. Omit a field to keep its default.
...
<ParamField body="template_id" type="string" default="scratch">
  Sandbox template id. Defaults to `scratch` (a minimal Ubuntu computer). Pass another id, such as `coding-agent`, to get a computer with extra tools preinstalled. In JavaScript, use `templateId`. See [Use a template](#use-a-template).
</ParamField>
...
A template is a ready-made computer image with extra tools already installed and sensible defaults for CPU, memory, and disk. Use one when the default `scratch` computer doesn't have what your agent needs out of the box.

For example, `coding-agent` includes common tools for coding tasks. Pass `template_id` (Python) or `templateId` (JavaScript) to pick a template:
...
You can still override the template's defaults by passing `cpus`, `memory`, or `disk_size_mb` alongside `template_id`.
...
Publishing a port gives your computer a public HTTPS URL that anyone with the link can reach. Use it when something running inside the sandbox — a web app, an API server, a Jupyter notebook — needs to be reachable from outside the computer.

The current release supports publishing port `8000`. If your app listens on a different port inside the computer, run it on `8000` (or proxy to it) before you publish.

<Note>
  Port publishing is currently available in the Python SDK and CLI only.
</Note>

```python Python
# Publish port 8000 and get a public URL
published = client.computers.publish_port(computer["id"])
print(published["url"])     # e.g., "https://p-abc123.celesto.ai"
print(published["status"])  # "publishing" or "published"

# List every public URL attached to a computer
for p in client.computers.list_published_ports(computer["id"]):
    print(f"{p['port']} -> {p['url']} ({p['status']})")
...
<ParamField body="port" type="integer" default="8000">
  Port inside the computer to expose. The current release supports `8000`.
</ParamField>

Each published port includes:

<ResponseField name="id" type="string">
  Unique identifier for the published port.
</ResponseField>

<ResponseField name="computer_id" type="string" required>
  ID of the computer the port belongs to.
</ResponseField>

<ResponseField name="port" type="integer" required>
  Port number inside the computer that is exposed.
</ResponseField>

<ResponseField name="url" type="string">
  Public HTTPS URL that maps to the port. Available once `status` is `published`.
</ResponseField>

<ResponseField name="status" type="string" required>
  One of: `publishing`, `published`, `unpublishing`, `unpublished`, `error`.
</ResponseField>

<ResponseField name="created_at" type="string">
  ISO 8601 timestamp of when the port was published.
</ResponseField>
...
Expose a web service running inside the computer and share the public URL:

```bash
# Start a web server inside the computer on port 8000, then:
celesto computer port publish einstein
# Prints: https://p-abc123.celesto.ai
