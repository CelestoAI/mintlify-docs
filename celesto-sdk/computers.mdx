...
<Note>
  Port publishing is currently available in the Python SDK and CLI. Use the [CLI port commands](/celesto-sdk/cli#port-commands) from TypeScript projects when you need to expose a service today.
</Note>
...
The TypeScript SDK provides a `getTerminalConnection` method that returns the URL, headers, and first message for an interactive WebSocket terminal session.
