...
<ParamField body="template_id" type="string" default="scratch">
  Sandbox template to create the computer from. Defaults to `scratch` (a minimal Ubuntu computer). Pass `coding-agent` when your agent needs common coding tools preinstalled. See [Use a template](/celesto-sdk/overview#use-a-template).
</ParamField>
...
<ParamField body="cpus" type="integer">
  Number of virtual CPUs for the new computer. Range: 1-16. Defaults to the template's value.
</ParamField>

<ParamField body="memory" type="integer">
  Memory in MB for the new computer. Range: 512-32768. Defaults to the template's value.
</ParamField>
